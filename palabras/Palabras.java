...
public class Palabras {
    public static void main(String[] args) {
        // Creamos las variables y ArrayLists necesarias para el
        // programa------------------------------------
        ...
        //Variable que sirve para comprobar por qué aparición de la palabra a borrar vamos.
        int contadorBorrar;
        contadorBorrar = 0;

        // Variables que sirven para guardar lo que hay en el fichero, los cambios sobre
        // él y el backup
        ...
        // ArrayLists y variables creadas, comienza el
        // programa:---------------------------------------------

        // Buscamos primero el nombre del fichero, para que el programa sepa sobre qué
        // fichero ha de actuar
        for (int i = 0; i < args.length; i++) {
            if (args[i].equals("-f")) {
                fEsta = true;
                posicionF = i;
                // Comprobamos que, además de estar -f, va seguido de algo (para que no salte
        ...
        // Dependiendo de si está o no el argumento "-f", se realizará una acción
        // diferente:
        if (fEsta) {
        ...
        // Una vez manejado el nombre del fichero a utilizar, podemos manejar el resto
        // de argumentos introducidos:------------------------
        for (int i = 0; i < args.length; i++) {
            switch (args[i]) {
                // ARGS DE BÚSQUEDA DE 1 PALABRA:
                // -----------------------------------------------------------------------
                // Buscar en el archivo la palabra que se encuentra junto a -c y contar su
                // número de apariciones
                case "-c":
                    //Comprobamos que hay un parámetro junto a -c para que no haya una IndexOutOfBoundsException
                    if(i < (args.length -1)){
                        palabra = args[++i];
                        contador = FuncionesBusqueda.contadorPalabra(partes, palabra);
                        FuncionesBusqueda.imprimirApariciones(contador, palabra);
                    } else {
                        System.out.println("Lo siento, para poder buscar la posición de una palabra se necesita introducir una palabra.");
                    }

                    break;
                // Buscar en el archivo la palabra que aparece junto -p, contar cuántas veces
                // está y decir la posición de la primera aparición de la palabra
                case "-p":
                    //Comprobamos que hay un parámetro junto a -p para que no haya una IndexOutOfBoundsException
                    if(i < (args.length -1)){
                        palabra = args[i + 1];
                    ...
                    break;
                // Buscar en el archivo la palabra que aparece junto -l, contar cuántas veces
                // está y decir la línea de la primera aparición de la palabra
                case "-l":
                    //Comprobamos que hay un parámetro junto a -l para que no haya una IndexOutOfBoundsException
                    if(i < (args.length -1)){
                        palabra = args[i + 1];
                        lineaPosicion = FuncionesBusqueda.lineaPalabra(palabra, partes);
                        // Comprobamos si la palabra está o no en el fichero para dar respuestas
                        // diferentes
                        if (lineaPosicion >= 0) {
                        ...
                        System.out.println("Lo siento, para poder buscar la línea de una palabra se necesita introducir una palabra.");
                    }
                ...
                // --------------------------------------------------
                // Contar las palabras que aparecen en el archivo (todas), mostrarlas ordenadas
                // por la cantidad de apariciones
                case "-a":
                ...
                case "-b":
                    //Comprobamos que hay una palabra junto a -b para que no haya una IndexOutOfBoundsException
                    if (i < (args.length - 1)) {
                        palabraBorrar = args[++i];

                        // Comprobar si hay -n para saber si debemos borrar todas las apariciones o la
                        // aparición número introducido por usuario
                        for (int i2 = 0; i2 < args.length; i2++) {
                            if (args[i2].equals("-n")) {
                                nEsta = true;
                                if (nEsta) {
                                    try {
                                        n = Integer.parseInt(args[++i2]);
                        ...
                        if (nEsta) {
                            ...
                            //Comprueba que la palabra esté en el fichero
                            if(FuncionesBusqueda.contadorPalabra(partes, palabraBorrar) >= 0){
                                //Comprueba que la palabra aparece n veces o más
                                if(FuncionesBusqueda.contadorPalabra(partes, palabraBorrar) >= n){
                                    //Borra la aparición n de la palabra
                                    for(int i2 = 0; i2 < partes.size(); i2++ ){
                                        if(partes.get(i2).contains(palabraBorrar)){
                                            contadorBorrar++;
                                            if(contadorBorrar == n){
                                                partes.remove(i2);
                                            }
                                ...
                                System.out.println("La palabra que intentas borrar no existe.");
                            }
                        } else {
                        ...
                        System.out.println("Lo siento, debe introducir una palabra junto a -b para que se pueda borrar.");
                    }
                    break;
